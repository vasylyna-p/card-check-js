...
let arrayWithoutLast = array.slice(0, array.length - 1);
let last = array[array.length - 1];
arrayWithoutLast.reverse();
for (i = 0; i < arrayWithoutLast.length; i++){
  if (i % 2 == 0){
    arrayWithoutLast[i] = arrayWithoutLast[i] * 2;
    if (arrayWithoutLast[i] > 9){
      arrayWithoutLast[i] = arrayWithoutLast[i] - 9;
    }
  }
}
let sum = arrayWithoutLast.reduce((previousValue, currentValue) => previousValue + currentValue) + last;
